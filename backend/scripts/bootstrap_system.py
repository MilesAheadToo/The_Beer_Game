#!/usr/bin/env python3
"""End-to-end bootstrap for the Daybreak Beer Game system."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-daybreak-games",
        action="store_true",
        help="Do not create or update the showcase Daybreak games.",
    )
    parser.add_argument(
        "--no-reset-games",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Timeout in seconds for the Daybreak LLM availability probe.",
    )
...
def bootstrap_system(args: argparse.Namespace) -> None:
    ...
    if args.agent_strategy:
        ...
        llm_available, llm_detail = check_daybreak_llm_access(
            model=args.llm_model,
            request_timeout=args.llm_timeout,
        )
        if llm_available:
            logger.info(
                "Daybreak LLM probe succeeded; default AI players will use the LLM strategy (%s).",
                args.llm_model or "auto",
            )
        else:
            logger.warning(
                "Daybreak LLM probe failed (%s); falling back to PI heuristic agents.",
                llm_detail,
            )
        preferred_strategy = "llm" if llm_available else "pi_heuristic"
    ...
    logger.info("[3/4] Verifying Daybreak Daybreak agent configuration...")
    if args.agent_strategy:
        logger.info("Default AI agents configured with explicit strategy '%s'.", args.agent_strategy)
    elif llm_available:
        logger.info("Default AI agents configured to use the Daybreak LLM strategy.")
    else:
        logger.info("Default AI agents configured to use the PI heuristic strategy.")
